  ...
  return <div className="container max-w-4xl mx-auto py-6 px-4">
      <Card className="border-twitter-blue shadow-md dark:bg-gray-800 dark:border-twitter-blue/50 overflow-hidden">
        <CardHeader className="bg-twitter-blue text-white">
          <CardTitle className="text-xl flex items-center md:text-2xl">
        ...
        <CardContent className="p-6 dark:bg-gray-800 px-3">
          <div className="space-y-4">
            ...
            <div className="relative">
              <Input type="password" placeholder="Enter your Gemini API key" value={apiKey} onChange={e => setApiKeyState(e.target.value)} className="bg-white border-gray-300 focus:border-twitter-blue dark:bg-gray-700 dark:border-gray-600 dark:text-white pr-10 font-mono text-sm" />
              <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-gray-400 dark:text-gray-500">
                API_KEY
              </span>
            </div>
            
            <Button variant="outline" className="w-full border-twitter-blue/30 text-twitter-blue flex items-center justify-center hover:bg-twitter-blue/10" onClick={() => setShowTutorial(!showTutorial)}>
              <HelpCircle className="h-4 w-4 mr-2" />
              {showTutorial ? "Hide API key instructions" : "How to get your Gemini API key"}
            </Button>
            
            {!freeTrialExhausted && <div className="flex items-start p-3 border border-amber-200 dark:border-amber-800 rounded-lg bg-amber-50 dark:bg-amber-900/20">
        ...
        <CardFooter className="flex justify-between bg-gray-50 dark:bg-gray-800 dark:border-t dark:border-gray-700 p-4">
          {!freeTrialExhausted && <Button onClick={handleSkip} variant="outline" className="dark:text-white dark:border-gray-600 dark:hover:bg-gray-700">
              Skip for now
              <ArrowRight className="ml-2 h-4 w-4" />
            </Button>}
          <Button onClick={handleSubmit} size="lg" className="bg-twitter-blue hover:bg-twitter-darkBlue dark:text-white" disabled={!apiKey.trim()}>
